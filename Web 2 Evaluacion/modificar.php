	...
	if(isset($_POST["btn-form"])){
		...
		if ($conn->connect_error) {
			die("Connexió KO: " . $conn->connect_error);
				...
				$sql = "UPDATE equips SET nom='".$nom."', escut='".$escut."', web='".$web."', localitat='".$localitat."' WHERE codi='".$codi."';";
		...
		<div class="collapse navbar-collapse" id="navbarNavDropdown">
			...
			<form action="modificar.php" method="POST">
				<div class="form-group">
					<label>Codigo</label>
					<input type="text" name="codi" class="form-control" placeholder="Codigo del equipo a modificar" required>
				</div>
				<br>
				<div class="form-group">
					<label>Nombre</label>
				...
				<div class="form-group">
					<label>Escudo</label>
					<input type="text" name="escudo" class="form-control" placeholder="Escudo">
				</div>
				<div class="form-group">
					<label>Localidad</label>
					<input type="text" name="localidad" class="form-control" placeholder="Localidad">
				</div>
				<button type="submit" name="btn-form" class="btn btn-success">Modificar</button>
			</form>
			<style>
			footer .glyphicon {
			  font-size: 20px;
